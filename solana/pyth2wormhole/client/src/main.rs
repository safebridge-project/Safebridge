...
fn handle_attest(
    ...
    let mut txs = Vec::with_capacity(attestation_cfg.symbols.len() / max_batch_size as usize);
        ...
        .chunks(max_batch_size as usize)
    {
        ...
        info!(
            ...
            symbols
                ...
                .map(|s| s
                    .name
                    .clone()
                    .unwrap_or(format!("unnamed product {:?}", s.product)))
                .collect::<Vec<_>>()
        );
        ...
        let mut sym_metas_vec: Vec<_> = symbols
            ...
            .map(|s| {
                vec![
                    AccountMeta::new_readonly(s.product, false),
                    AccountMeta::new_readonly(s.price, false),
                ]
            })
            .flatten()
            .collect();
